...
- **合并锚点（必读，4 个并行 agent 都会触碰）**：`core/runner.py::_build_registry()` 和 `core/permissions/policy.py::DEFAULT_POLICIES` 是 4 个包 agent（包 B 工具搜索、包 C 交互/文件安全、包 D 计划模式/钩子、包 F 团队协调）都要修改的公共登记点。**新工具注册行上方必须加一行注释 `# <tool_name>（agent: package-c）`** 标识本包归属，方便第 1 波合并时按注释去重，不会出现"两个包都注册了 ask_user"的冲突。例如：
  ...
  ```
- **edit_file.py 修改位置约束（必读）**：包 C 的 C5 任务要修改 `core/tools/builtin/edit_file.py` 接入 staleness 检查，**但基础闭环计划（`2026-07-20-kama-agent-minimal-loop.md`）Task 4 的另一个 agent 也会改这个文件**（它定义了 `class EditFileTool` 和 `@staticmethod _atomic_write`）。两个 agent 并行执行时，包 C 的 C5 任务必须等基础闭环 Task 4 的 `_atomic_write` 方法落地后再做合并/集成，**新加的 staleness 检查方法 `_check_staleness` 放在 `edit_file.py` 里 `_atomic_write` 静态方法的下方**（文件位置明确），并在 `invoke()` 方法的早期（参数校验后、文件读取前）调用。这样第 1 波集成时按文件行号定位就能把两边的改动拼起来。
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/ask_user.py`
...
Expected: FAIL（`ModuleNotFoundError: No module named 'kama_claude.core.tools.builtin.ask_user'`）

- [ ] **Step 3: 实现 QuestionStore + AskUserTool**

```python
# src/kama_claude/core/tools/builtin/ask_user.py
from __future__ import annotations
...
Run: `uv run ruff check src/kama_claude/core/tools/builtin/ask_user.py tests/unit/test_ask_user_tool.py`
Run: `uv run mypy src/kama_claude/core/tools/builtin/ask_user.py`
Expected: 全部通过

- [ ] **Step 6: 提交**

```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/tools/builtin/ask_user.py tests/unit/test_ask_user_tool.py
...
**Files:**
- Create: `src/kama_claude/tui/ask_user_dialog.py`
...
Expected: FAIL（`ModuleNotFoundError: No module named 'kama_claude.tui.ask_user_dialog'`）

- [ ] **Step 3: 实现 AskUserDialog**

```python
# src/kama_claude/tui/ask_user_dialog.py
from __future__ import annotations
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/tui/ask_user_dialog.py tests/unit/test_ask_user_dialog.py
...
**Files:**
- Modify: `src/kama_claude/core/runner.py`（在 `_build_registry` 里注册 `AskUserTool`，加 `# ask_user（agent: package-c）` 锚点注释）
- Modify: `src/kama_claude/core/permissions/policy.py::DEFAULT_POLICIES`（登记 `ask_user` 策略 + 在 `_PREVIEW_KEY` 加预览字段）
- Modify: `src/kama_claude/core/permissions/manager.py`（让 `eval_ask_user_tools` 走挂起路径——ask_user 总是 ASK，所以默认就落进 `await future`；**不需要改 manager**）
- Modify: `src/kama_claude/core/bus/events.py`（加 `AskUserRequestedEvent`，并把 `Event` 联合加上）
- Modify: `src/kama_claude/tui/app.py`（订阅 `ask_user.requested` 事件，调 `mount_dialog`，监听 `Answered` 消息并通过 IPC 回包到 `question_store.respond()`）
...
在 `src/kama_claude/core/bus/events.py` 末尾追加（`PermissionDeniedEvent` 之后）：
...
在 `core/runner.py` 顶部 `from kama_claude.core.tools.builtin import (...)` 块里加 `AskUserTool`：
...
在 `src/kama_claude/core/permissions/policy.py` 的 `DEFAULT_POLICIES` 字典里加：
...
在 `src/kama_claude/tui/app.py` 的事件订阅 `topics` 列表里追加 `"ask_user.*"`：
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/runner.py src/kama_claude/core/permissions/policy.py \
        src/kama_claude/core/bus/events.py src/kama_claude/tui/app.py \
...
**Files:**
- Create: `src/kama_claude/core/tools/file_state_cache.py`
- Test: `tests/unit/test_file_state_cache.py`
- Modify: `src/kama_claude/core/tools/builtin/read_file.py`（在读取成功后调 `cache.record(path)`）
...
```python
# src/kama_claude/core/tools/file_state_cache.py
from __future__ import annotations
...
修改 `src/kama_claude/core/tools/builtin/read_file.py`，把 `ReadFileTool` 改为可注入 cache，并把 `record` 调用放在读取成功后：
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/tools/file_state_cache.py \
        src/kama_claude/core/tools/builtin/read_file.py \
        tests/unit/test_file_state_cache.py
...
**Files:**
- Modify: `src/kama_claude/core/tools/builtin/edit_file.py`（在 `_atomic_write` 静态方法**下方**追加 `_check_staleness` 方法；`invoke()` 早期调用一次）
- Modify: `src/kama_claude/core/runner.py`（把 `FileStateCache` 注入 `ReadFileTool` 和 `EditFileTool`，两个工具共享同一份 cache）
...
修改 `src/kama_claude/core/tools/builtin/edit_file.py`：
...
修改 `src/kama_claude/core/runner.py`：
...
并在 `from kama_claude.core.tools.builtin import (...)` 块里加 `EditFileTool`：
...
**注意**：本步骤假设 `EditFileTool` 已经在基础闭环计划（`2026-07-20-kama-agent-minimal-loop.md` Task 4）里落地。如果基础闭环 Task 4 还没合并，本步骤的 EditFileTool 导入会失败——届时把这一行注释掉、`if _ok("edit_file")` 整段用 `# fmt: off` 跳过，等基础闭环合并后回来补。
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/tools/builtin/edit_file.py \
...
**Files:**
- Create: `src/kama_claude/core/filehistory/history.py`
- Create: `src/kama_claude/core/filehistory/__init__.py`
...
from kama_claude.core.filehistory.history import FileHistory, FileSnapshot
...
Expected: FAIL（`ModuleNotFoundError: No module named 'kama_claude.core.filehistory'`）

- [ ] **Step 3: 实现 FileHistory**

```python
# src/kama_claude/core/filehistory/__init__.py
from kama_claude.core.filehistory.history import FileHistory, FileSnapshot
...
```python
# src/kama_claude/core/filehistory/history.py
from __future__ import annotations
...
Run: `uv run ruff check src/kama_claude/core/filehistory tests/unit/test_file_history.py`
Run: `uv run mypy src/kama_claude/core/filehistory`
Expected: 全部通过
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/filehistory/ tests/unit/test_file_history.py
...
**Files:**
- Modify: `src/kama_claude/core/filehistory/history.py`（加 `rewind(path, version) -> None`）
- Create: `src/kama_claude/core/tools/builtin/rewind_file.py`
- Modify: `src/kama_claude/core/runner.py`（注入 `FileHistory`、注册 `rewind_file` 工具）
- Modify: `src/kama_claude/core/permissions/policy.py::DEFAULT_POLICIES`（登记 `rewind_file` 策略）
...
from kama_claude.core.tools.builtin.rewind_file import RewindFileTool
...
在 `src/kama_claude/core/filehistory/history.py` 末尾追加（`get_path` 后面）：
...
```python
# src/kama_claude/core/tools/builtin/rewind_file.py
from __future__ import annotations
...
并在 `from kama_claude.core.tools.builtin import (...)` 块里加 `RewindFileTool`：
...
**注意**：`FileHistory` 路径硬编码 `Path.cwd() / ".kama" / "file-history"` 在 runner 启动时一次性求值；如果你后续要让 `FileHistory` 也支持 per-session 目录（类似 `note_save` 的 `store, session.id, run_id` 注入），单独在另一个 task 改。本任务只做"单实例 cwd 下 `.kama/file-history/`"。
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/kama-package-c-plan"
git add src/kama_claude/core/filehistory/history.py \
        src/kama_claude/core/tools/builtin/rewind_file.py \
        src/kama_claude/core/runner.py \
...
Run: `grep -n "agent: package-c" src/kama_claude/core/runner.py src/kama_claude/core/permissions/policy.py`
Expected: 至少命中以下行：
...
Run: `grep -n "_atomic_write\|_check_staleness" src/kama_claude/core/tools/builtin/edit_file.py`
