...
**Files:**
- Create: `src/kama_claude/tui/permission_widgets.py`
- Modify: `src/kama_claude/tui/app.py`
...
```python
# src/kama_claude/tui/permission_widgets.py
...
Run: `uv run ruff check src/kama_claude/tui/`
Run: `uv run mypy src/kama_claude/tui/`
Expected: 无新增问题
...
git add src/kama_claude/tui/app.py src/kama_claude/tui/permission_widgets.py
...
**Files:**
- Modify: `src/kama_claude/core/session/store.py`
...
```bash
git add src/kama_claude/core/session/store.py tests/unit/test_session_store.py
...
**Files:**
- Create: `src/kama_claude/tui/session_screen.py`
...
```python
# src/kama_claude/tui/session_screen.py
from __future__ import annotations
...
删除 `class SessionSelected` 那段无意义占位。若要支持"选中后恢复"，用 Textual 内建的 `ListView`/`ListItem` 或者给每行 `Static` 加 `can_focus=True` + 键盘事件处理，选中后调用 `self.dismiss(session.id)`——具体交互细节实现时可以先做最小可用版本（只读展示，不支持键盘选择），恢复动作留给用户手动执行 `kama-tui --replay <run_id>`（已有能力），本任务的核心交付是"能看到所有会话和它们的进度"，交互式选择可以是后续独立的小任务。
...
```bash
git add src/kama_claude/tui/session_screen.py src/kama_claude/tui/app.py tests/unit/test_session_screen.py
...
- Modify: `src/kama_claude/core/teams/manager.py`
...
```bash
git add src/kama_claude/core/bus/events.py src/kama_claude/core/teams/manager.py \
...
**Files:**
- Create: `src/kama_claude/tui/team_tree.py`
...
```python
# src/kama_claude/tui/team_tree.py
from __future__ import annotations
...
```bash
git add src/kama_claude/tui/team_tree.py src/kama_claude/tui/app.py tests/unit/test_team_tree.py
...
**Files:**
- Modify: `src/kama_claude/core/bus/commands.py`
...
顶部补 import：`from kama_claude.core.bus.commands import SetPermissionModeCommand, SetPermissionModeResult`、`from kama_claude.core.permissions.modes import PermissionMode`。
...
```bash
git add src/kama_claude/core/bus/commands.py src/kama_claude/core/app.py \
...
**Files:**
- Create: `src/kama_claude/tui/plan_dialog.py`
...
```python
# src/kama_claude/tui/plan_dialog.py
from __future__ import annotations
...
```bash
git add src/kama_claude/tui/plan_dialog.py src/kama_claude/tui/app.py tests/unit/test_plan_dialog.py
