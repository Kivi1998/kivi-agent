...
**Goal:** 给 KamaClaude 已有的上下文压缩（`Compactor`）加固三层韧性（熔断、细粒度替换记录、落盘式工具输出预算），并新增一个跨 session 的长期记忆模块（抽取、存储、召回），解决"长任务不稳定""每次对话都从零开始"两个真实痛点——这是原企业整合方案里唯一被标注 P0 的个人可做项。

**Architecture:** 不重写 `Compactor`，在其现有 `compact()`/`compact_messages()` 之外新增失败计数状态和一个新的细粒度记录模块；长期记忆完全独立于 session 的 thread/notes 机制，是第三层记忆（`session 内 notes.md` < `项目/用户 context.md` < `跨 session 长期记忆`），存储在 `~/.kama/memory/` 下的独立 Markdown 文件，通过 `ExecutionContext` 新增字段注入 system prompt。
...
**Files:**
- Modify: `src/kama_claude/core/compact/compactor.py`
- Modify: `src/kama_claude/core/bus/events.py`
- Modify: `src/kama_claude/core/loop.py`
...
git add src/kama_claude/core/compact/compactor.py src/kama_claude/core/bus/events.py tests/unit/test_compactor.py
...
**Files:**
- Create: `src/kama_claude/core/session/replacement.py`
- Modify: `src/kama_claude/core/compact/compactor.py`
...
```python
# src/kama_claude/core/session/replacement.py
from __future__ import annotations
...
```python
        from kama_claude.core.session.replacement import ReplacementRecord, write_replacement_record
        write_replacement_record(
            self._session_dir,
            ReplacementRecord(
                ts=_now(),
...
```bash
git add src/kama_claude/core/session/replacement.py src/kama_claude/core/compact/compactor.py \
        tests/unit/test_replacement_record.py
...
**Files:**
- Create: `src/kama_claude/core/session/checkpoint.py`
...
```python
# src/kama_claude/core/session/checkpoint.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/session/checkpoint.py src/kama_claude/core/runner.py tests/unit/test_checkpoint.py
...
**Files:**
- Modify: `src/kama_claude/core/compact/budget.py`
...
from kama_claude.core.compact.budget import persist_and_truncate_tool_results
...
```python
# src/kama_claude/core/compact/budget.py 追加
import hashlib
from pathlib import Path
...
```bash
git add src/kama_claude/core/compact/budget.py tests/unit/test_budget.py
...
**Files:**
- Create: `src/kama_claude/core/memory/store.py`
...
```python
# src/kama_claude/core/memory/store.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/memory/store.py tests/unit/test_memory_store.py
...
**Files:**
- Create: `src/kama_claude/core/memory/recall.py`
- Modify: `src/kama_claude/core/context.py`
...
```python
# src/kama_claude/core/memory/recall.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/memory/recall.py src/kama_claude/core/context.py \
        src/kama_claude/core/runner.py tests/unit/test_memory_recall.py
...
**Files:**
- Create: `src/kama_claude/core/memory/extractor.py`
- Modify: `src/kama_claude/core/runner.py`
...
```python
# src/kama_claude/core/memory/extractor.py
from __future__ import annotations
...
async def extract_memories(
    ...
    try:
        response = await provider.chat(  # type: ignore[attr-defined]
            ...
            tool_schemas=[],
            bus=__import__("kama_claude.core.events.bus", fromlist=["EventBus"]).EventBus(),
...
```bash
git add src/kama_claude/core/memory/extractor.py src/kama_claude/core/runner.py tests/unit/test_memory_extractor.py
