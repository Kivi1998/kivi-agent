...
**Goal:** 在 KamaClaude（stage/s7）基础上，迁移 mewcode 的四类关键能力（真实编程工具、多模型 Provider、Git 工作树隔离、命令沙箱），跑通《kamaAgent 企业 Agent 平台整合方案》第 21 章"首轮落地建议"描述的最小闭环：用户在 TUI 发起一个真实代码修改任务 → Agent 用搜索/精确编辑/差异工具完成修改 → 修改发生在独立 Git 工作树内 → 高风险命令经沙箱执行并触发审批 → 用户看差异决定接受。

**Architecture:** 不新建仓库、不引入服务化/多前端/企业治理（那些是团队级任务，不适合个人执行）。所有新增能力都以 KamaClaude 现有扩展点接入：新工具继承 `core/tools/builtin/` 下的 `BaseTool`，新 Provider 实现 `core/llm/base.py` 的 `LLMProvider` Protocol，工作树与沙箱各自成一个新包（`core/workspace/`、`core/sandbox/`），最终都在 `core/runner.py::_build_registry()` 注册、在 `core/permissions/policy.py::DEFAULT_POLICIES` 登记权限策略。mewcode 没有 LICENSE 文件，因此只迁移设计思路、在 KamaClaude 里用自己的代码风格重写，不直接复制 mewcode 源码。
...
- 不新建仓库：在现有 `Kama/KamaClaude` 仓库基于 `stage/s7` 建分支 `feat/kama-agent-minimal-loop`。
- mewcode 无 LICENSE 文件：只迁移算法/架构思路，所有代码在 KamaClaude 中独立实现，禁止逐段复制 mewcode 源码。
...
git checkout -b feat/kama-agent-minimal-loop
```
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/_fs_filters.py`
- Create: `src/kama_claude/core/tools/builtin/glob_tool.py`
- Test: `tests/unit/test_glob_tool.py`
- Modify: `src/kama_claude/core/runner.py`（`_build_registry` 内注册）
- Modify: `src/kama_claude/core/permissions/policy.py`（`DEFAULT_POLICIES` 登记）
...
```python
# src/kama_claude/core/tools/builtin/_fs_filters.py
...
Expected: FAIL（`ModuleNotFoundError: No module named 'kama_claude.core.tools.builtin.glob_tool'`）

- [ ] **Step 4: 实现 GlobTool**

```python
# src/kama_claude/core/tools/builtin/glob_tool.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/tools/builtin/_fs_filters.py \
        src/kama_claude/core/tools/builtin/glob_tool.py \
        tests/unit/test_glob_tool.py \
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/grep_tool.py`
...
```python
# src/kama_claude/core/tools/builtin/grep_tool.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/tools/builtin/grep_tool.py tests/unit/test_grep_tool.py \
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/edit_file.py`
...
```python
# src/kama_claude/core/tools/builtin/edit_file.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/tools/builtin/edit_file.py tests/unit/test_edit_file_tool.py \
        src/kama_claude/core/runner.py src/kama_claude/core/permissions/policy.py
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/diff_tool.py`
...
```python
# src/kama_claude/core/tools/builtin/diff_tool.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/tools/builtin/diff_tool.py tests/unit/test_diff_tool.py \
        src/kama_claude/core/runner.py src/kama_claude/core/permissions/policy.py
...
- Create: `src/kama_claude/core/llm/openai_compat_provider.py`
...
```python
# src/kama_claude/core/llm/openai_compat_provider.py
from __future__ import annotations
...
```bash
git add pyproject.toml uv.lock src/kama_claude/core/llm/openai_compat_provider.py \
        tests/unit/test_openai_compat_provider.py
...
- Modify: `src/kama_claude/core/runner.py`
- Modify: `src/kama_claude/core/app.py`
...
```python
# src/kama_claude/core/llm/factory.py
from __future__ import annotations
...
并把 import 从 `from kama_claude.core.llm.provider import AnthropicProvider` 改为 `from kama_claude.core.llm.factory import build_provider`（若其他地方仍用 `AnthropicProvider` 保留原 import）。
...
并加 `from kama_claude.core.llm.factory import build_provider`。
...
```bash
git add src/kama_claude/core/config.py src/kama_claude/core/llm/factory.py \
        src/kama_claude/core/runner.py src/kama_claude/core/app.py tests/unit/test_config_env.py
...
**Files:**
- Create: `src/kama_claude/core/workspace/__init__.py`
- Create: `src/kama_claude/core/workspace/worktree.py`
...
```python
# src/kama_claude/core/workspace/worktree.py
from __future__ import annotations
...
async def create_worktree(repo_root: Path, name: str, base_branch: str = "HEAD") -> WorktreeInfo:
    slug = slugify(name)
    branch = f"kama-agent-{slug}"
    worktree_path = repo_root / ".kama" / "worktrees" / slug
...
```python
# src/kama_claude/core/workspace/__init__.py
```
...
```bash
git add src/kama_claude/core/workspace/ tests/unit/test_worktree.py
...
**Files:**
- Create: `src/kama_claude/core/tools/builtin/enter_worktree.py`
- Create: `src/kama_claude/core/tools/builtin/exit_worktree.py`
...
```python
# src/kama_claude/core/tools/builtin/enter_worktree.py
from __future__ import annotations
...
from kama_claude.core.tools.base import BaseTool, ToolResult
from kama_claude.core.workspace.worktree import create_worktree
...
```python
# src/kama_claude/core/tools/builtin/exit_worktree.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/tools/builtin/enter_worktree.py \
        src/kama_claude/core/tools/builtin/exit_worktree.py \
        tests/unit/test_worktree_tools.py \
        src/kama_claude/core/runner.py src/kama_claude/core/permissions/policy.py
...
**Files:**
- Create: `src/kama_claude/core/sandbox/__init__.py`
- Create: `src/kama_claude/core/sandbox/seatbelt.py`
- Create: `src/kama_claude/core/sandbox/bwrap.py`
...
```python
# src/kama_claude/core/sandbox/seatbelt.py
from __future__ import annotations
...
```python
# src/kama_claude/core/sandbox/bwrap.py
from __future__ import annotations
...
```python
# src/kama_claude/core/sandbox/__init__.py
from __future__ import annotations
...
```bash
git add src/kama_claude/core/sandbox/ tests/unit/test_sandbox.py
...
**Files:**
- Modify: `src/kama_claude/core/tools/builtin/bash.py`
- Modify: `src/kama_claude/core/runner.py`
...
```bash
cd "/Users/kivi/Documents/agent系统/Kama/KamaClaude"
uv run kama-core &            # 启动 daemon
uv run kama-tui                # 打开 TUI，发起一个真实代码修改目标，例如：
...
7. 全过程在 `~/.kama/sessions/<sid>/events.jsonl` 里留下完整可回放事件
...
```bash
git add src/kama_claude/core/tools/builtin/bash.py src/kama_claude/core/runner.py \
...
- **覆盖范围**：11 个任务覆盖了《整合方案》第 21 章"首轮落地建议"列出的全部 7 个验收点（多模型 Provider、真实编程工具链、工作树隔离、沙箱执行、可恢复审批、diff 审查、事件可回放——最后一项复用 KamaClaude 已有的 `EventWriter`，不需要新任务）。
