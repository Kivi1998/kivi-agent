...
**Goal:** 给 kivi-agent 加两块能力：① 权限模式（`PermissionMode`：DEFAULT/ACCEPT_EDITS/PLAN/BYPASS），在现有 `PermissionDecision(ALLOW/DENY/ASK)` 三态评估之上加一层模式覆盖；② 生命周期钩子系统，让工具调用前可拒绝、调用后可异步触发自定义逻辑，为后续包 F（团队协调策略）提供扩展点。
...
**Files:**
- Modify: `src/kivi_agent/core/tools/base.py`
- Modify: `src/kivi_agent/core/tools/builtin/read_file.py`、`list_dir.py`、`write_file.py`、`bash.py`、`note_save.py`、`task_create.py`、`task_get.py`、`task_list.py`、`task_update.py`
...
git add src/kivi_agent/core/tools/base.py src/kivi_agent/core/tools/builtin/ tests/unit/test_tool_categories.py
...
**Files:**
- Create: `src/kivi_agent/core/permissions/modes.py`
...
```python
# src/kivi_agent/core/permissions/modes.py
from __future__ import annotations
...
```bash
git add src/kivi_agent/core/permissions/modes.py tests/unit/test_permission_modes.py
...
**Files:**
- Modify: `src/kivi_agent/core/permissions/manager.py`
...
```python
from kivi_agent.core.permissions.modes import PermissionMode, mode_override
from kivi_agent.core.tools.registry import ToolRegistry  # 用于按工具名查 category，见下方说明
```
...
```bash
git add src/kivi_agent/core/permissions/manager.py src/kivi_agent/core/runner.py tests/unit/test_permission_manager.py
...
**Files:**
- Create: `src/kivi_agent/core/tools/builtin/exit_plan_mode.py`
...
```python
# src/kivi_agent/core/tools/builtin/exit_plan_mode.py
from __future__ import annotations
...
```bash
git add src/kivi_agent/core/tools/builtin/exit_plan_mode.py tests/unit/test_exit_plan_mode_tool.py \
...
**Files:**
- Create: `src/kivi_agent/core/hooks/__init__.py`
- Create: `src/kivi_agent/core/hooks/events.py`
- Create: `src/kivi_agent/core/hooks/models.py`
...
```python
# src/kivi_agent/core/hooks/events.py
from __future__ import annotations
...
```python
# src/kivi_agent/core/hooks/models.py
from __future__ import annotations
...
```python
# src/kivi_agent/core/hooks/__init__.py
```
...
```bash
git add src/kivi_agent/core/hooks/ tests/unit/test_hook_models.py
...
**Files:**
- Create: `src/kivi_agent/core/hooks/engine.py`
- Modify: `src/kivi_agent/core/tools/errors.py`
...
```python
# src/kivi_agent/core/hooks/engine.py
from __future__ import annotations
...
```bash
git add src/kivi_agent/core/hooks/engine.py src/kivi_agent/core/tools/errors.py tests/unit/test_hook_engine.py
...
**Files:**
- Modify: `src/kivi_agent/core/tools/invocation.py`
...
顶部加 `from kivi_agent.core.hooks.engine import HookEngine`（放在 `TYPE_CHECKING` 块内，因为函数签名里用字符串字面量类型标注，避免与 `core/hooks` 之间产生不必要的运行时循环 import 风险）。
...
```bash
git add src/kivi_agent/core/tools/invocation.py tests/unit/test_invocation.py
...
- Create: `src/kivi_agent/core/hooks/loader.py`
- Modify: `src/kivi_agent/core/runner.py`
...
from kivi_agent.core.config import HooksConfig, HookEntry
from kivi_agent.core.hooks.events import LifecycleEvent
...
```python
# src/kivi_agent/core/hooks/loader.py
from __future__ import annotations
...
```bash
git add src/kivi_agent/core/config.py src/kivi_agent/core/hooks/loader.py \
        src/kivi_agent/core/runner.py tests/unit/test_hook_loader.py
