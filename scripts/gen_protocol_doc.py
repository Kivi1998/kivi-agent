...
def generate() -> str:
    ...
    sections = [
        ...
        "\n",
        _model_section("SetPermissionModeCommand", SetPermissionModeCommand,
            {"type": "permission.set_mode", "session_id": session_id, "mode": "default"}),
        "\n",
        _model_section("SetPermissionModeResult", SetPermissionModeResult,
            {"ok": True}),
        "\n",
        _model_section("SessionCancelCommand", SessionCancelCommand,
            {"type": "session.cancel", "session_id": session_id, "reason": "user_requested"}),
        "\n",
        _model_section("SessionCancelResult", SessionCancelResult,
            {"session_id": session_id, "cancelled": True, "ts": ts}),
        "\n## Server Push\n\n",
        ...
        _model_section("LlmThinkingEvent", LlmThinkingEvent,
            {"type": "llm.thinking", "run_id": run_id, "step": 1,
             "content": "Let me reason about this...", "ts": ts}),
        "\n",
        ...
        "\n",
        _model_section("ChartRenderedEvent", ChartRenderedEvent,
            {"type": "chart.rendered", "run_id": run_id, "chart_id": "chart-1",
             "option_dict": {"xAxis": {"type": "category"}, "series": [{"type": "bar"}]},
             "ts": ts}),
        "\n",
        _model_section("RagSourcesCitedEvent", RagSourcesCitedEvent,
            {"type": "rag.sources_cited", "run_id": run_id,
             "sources": [{"id": "doc-1", "title": "Source 1"}, {"id": "doc-2", "title": "Source 2"}],
             "ts": ts}),
        "\n",
        _model_section("FrontendToolCallRequested", FrontendToolCallRequested,
            {"type": "frontend.tool_call_requested", "run_id": run_id,
             "request_id": "req-1", "tool_name": "open_url",
             "args": {"url": "https://example.com"}, "ts": ts}),
        "\n",
        _model_section("FrontendToolCallResponded", FrontendToolCallResponded,
            {"type": "frontend.tool_call_responded", "run_id": run_id,
             "request_id": "req-1", "result": {"ok": True, "data": [1, 2, 3]},
             "ts": ts}),
        "\n",
        _model_section("RunCancelledEvent", RunCancelledEvent,
            {"type": "run.cancelled", "run_id": run_id,
             "reason": "user_requested", "ts": ts}),
        "\n## Session Events\n\n",
        ...
        _model_section("TeamCreatedEvent", TeamCreatedEvent,
            {"type": "team.created", "team_id": "team-abc12345", "goal": "重构登录模块",
             "members": [{"name": "planner", "role": "planner", "run_id": "run-aaa"},
                         {"name": "executor", "role": "executor", "run_id": "run-bbb"}],
             "ts": ts}),
        "\n## Error Codes\n\n",
        "| Code | Name | Meaning |\n",
        "|------|------|---------|\n",
