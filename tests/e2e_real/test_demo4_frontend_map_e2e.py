"""Demo 4 前端操作 Agent 真实 LLM 端到端（agent: package-e2e-real-w82）。

# test_demo4_frontend_map_e2e.py（agent: package-e2e-real-w82）
Wave 8.2 WT-L3：跑 ``demos.demo4_frontend_map`` + 真实 httpx fetch GeoJSON。

env guard：``KIVI_RUN_E2E != "1"`` 时整个 module skip（默认安全）。
真实模式：设 ``KIVI_REAL_GEOJSON=1`` 让 demo 走真实 httpx fetch 路径。
"""
from __future__ import annotations

import os

import pytest
from demos.demo4_frontend_map import Demo4FrontendMap

from tests.e2e_real.report import E2ERunResult, make_run_result

pytestmark = pytest.mark.skipif(
    os.environ.get("KIVI_RUN_E2E") != "1",
    reason="KIVI_RUN_E2E != 1; set KIVI_RUN_E2E=1 to run real LLM e2e",
)


# 拼装单次 E2ERunResult（agent: package-e2e-real-w82）
def _build_result(
    duration_seconds: float, success: bool, summary: str, error: str | None
) -> E2ERunResult:
    """从 demo 结果构造标准 E2ERunResult。"""
    from tests.e2e_real.conftest import resolve_provider_name  # noqa: PLC0415

    return make_run_result(
        provider=resolve_provider_name(),
        model=os.environ.get("KIVI_LLM_DEFAULT_MODEL", "claude-sonnet-4-6"),
        case_id="demo4_frontend_map",
        case_name="前端操作 Agent：GeoJSON 地图加载",
        duration_seconds=duration_seconds,
        input_tokens=0,
        output_tokens=0,
        success=success,
        output_preview=summary,
        error=error,
    )


# Demo 4 真实 LLM 跑通（agent: package-e2e-real-w82）
async def test_demo4_frontend_map_e2e(e2e_report) -> None:
    """跑 ``Demo4FrontendMap`` + 记录 E2ERunResult。"""
    async with Demo4FrontendMap() as demo:
        result = await demo.execute()

    e2e_report.add(
        _build_result(
            duration_seconds=result.duration_seconds,
            success=result.status == "passed",
            summary=result.summary,
            error=result.error,
        )
    )
    assert result.status in ("passed", "failed"), result.summary
