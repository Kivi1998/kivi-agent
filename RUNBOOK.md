...
2. [启停操作](#2-启停操作)
3. [开发命令](#3-开发命令)
4. [真实 LLM 端到端（Wave 8.2 新增）](#4-真实-llm-端到端wave-82-新增)
5. [故障排查（7 场景）](#5-故障排查7-场景)
6. [监控 / 日志](#6-监控--日志)
7. [升级 / 迁移指南](#7-升级--迁移指南)
...
## 4. 真实 LLM 端到端（Wave 8.2 新增）

> **本节定位**：运维视角的"怎么让真实 LLM 跑通"。完整用户文档参见 [docs/e2e-real/README.md](./docs/e2e-real/README.md)。
> **核心原则**：默认**全部跳过**（env guard `KIVI_RUN_E2E=1` 才跑），**不主动扣 token**。
> **Wave 8.2 范围**：L1 AnthropicProvider 增强 + L2 OpenAICompatProvider 增强 + L3 真实 LLM E2E 框架 + L4 文档。

### 4.1 L1/L2 增强一览

| Provider | 增强项 | env var |
|---|---|---|
| **AnthropicProvider**（`src/kivi_agent/core/llm/provider.py`） | 重试（429 / 5xx exponential backoff 1s / 2s / 4s） / 超时 / Token 统计（input / output / total）/ 流式（`stream_complete` async generator）/ 错误归一化（`LLMRateLimitError` / `LLMTimeoutError` / `LLMUnavailableError`） | `KIVI_ANTHROPIC_API_KEY` / `KIVI_ANTHROPIC_BASE_URL` |
| **OpenAICompatProvider**（`src/kivi_agent/core/llm/openai_compat_provider.py`） | 同上 + DeepSeek 兼容（`base_url` 可配 `/v1` 端点）/ OpenAI Embedding batch（`KIVI_EMBEDDING_BATCH_SIZE`） | `KIVI_OPENAI_API_KEY` / `KIVI_OPENAI_BASE_URL` / `KIVI_OPENAI_MODEL` |
| **全 provider 共享** | 默认 model / 超时 / 重试 | `KIVI_LLM_DEFAULT_MODEL` / `KIVI_LLM_TIMEOUT` / `KIVI_LLM_MAX_RETRIES` |

**对比 Wave 1 的简单实现**：

- Wave 1：基本 async 调用 + token 跟踪，**无重试 / 无超时配置 / 无流式**
- Wave 8.2：完整生产级（重试 + 超时 + Token + 流式 + 错误归一化 + config 暴露）

**默认行为**（无 env var）：

- `KIVI_LLM_TIMEOUT=30` 秒
- `KIVI_LLM_MAX_RETRIES=3`（exponential backoff 1s / 2s / 4s）
- 错误归一化后抛 `LLMRateLimitError` / `LLMTimeoutError` / `LLMUnavailableError`（替代原始 SDK exception）
...
```bash
# .env
KIVI_ANTHROPIC_API_KEY=sk-ant-...              # 必填
# KIVI_ANTHROPIC_BASE_URL=https://api.anthropic.com  # 可选（中转/DeepSeek 兼容）
```
...
```bash
# .env
KIVI_OPENAI_API_KEY=sk-...                       # 必填
# KIVI_OPENAI_BASE_URL=https://api.openai.com/v1  # 可选（默认 OpenAI；DeepSeek / 自部署替换）
KIVI_OPENAI_MODEL=gpt-4o-mini                    # 可选（默认 gpt-4o-mini）
```
...
```bash
# .env
KIVI_LLM_DEFAULT_MODEL=claude-sonnet-4-6         # factory.create_provider() 未指定时用
KIVI_LLM_TIMEOUT=30                              # 单次 LLM 调用超时（秒）
KIVI_LLM_MAX_RETRIES=3                           # 429 / 5xx 自动重试次数
```

#### (4) E2E 测试 env guard

```bash
# .env（默认全部跳过；用户 export 后才跑）
# KIVI_RUN_E2E=1                                # 必须显式打开
KIVI_E2E_PROVIDER=anthropic                     # anthropic | openai
KIVI_E2E_MAX_CASES=5                            # 限制 case 数量（防 token 失控）
KIVI_E2E_REPORT_DIR=reports/e2e_real/            # 报告输出目录
```

> **完整 env var 清单**见 [.env.example](./.env.example)（Wave 8.2 增 5 段：`[Anthropic]` / `[OpenAI 兼容]` / `[LLM 通用]` / `[Embedding]` / `[E2E Real LLM]`）。
...
#### Step 2：export key + 启用 E2E

```bash
# 选一种 provider
export KIVI_ANTHROPIC_API_KEY=sk-ant-api03-xxxxxx
# 或
# export KIVI_OPENAI_API_KEY=sk-proj-xxxxxx
...
- `case_id` / `case_name` / `provider` / `model`
- `started_at` / `duration_seconds` / `latency_seconds`
- `input_tokens` / `output_tokens` / `total_tokens` / `cost_usd`
- `success` / `error` / `output_quality_score`（人工 0-5）
- `output_preview`（前 200 字符）

### 4.5 怎么控制成本

```bash
# 1. 限制 case 数量（最少 token 验证框架）
export KIVI_E2E_MAX_CASES=2

# 2. 选最便宜的 provider
# DeepSeek（~Anthropic 1/30）
export KIVI_ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic
export KIVI_LLM_DEFAULT_MODEL=deepseek-chat

# 3. 选最便宜的 model
export KIVI_OPENAI_MODEL=gpt-4o-mini   # ~claude-sonnet-4-6 价格 1/20

# 4. 估算法（5 demo + 5 eval = 10 case）
# Anthropic: ~$0.10
...
#### 场景 A：401 Unauthorized

```bash
# 排查
echo $KIVI_ANTHROPIC_API_KEY | head -c 12   # 应输出 sk-ant-api03
...
**修复**：重新从 Anthropic Console 复制 key；确认是 `sk-ant-...` 前缀。

#### 场景 B：429 Too Many Requests（限流）

**L1/L2 已自动 exponential backoff 重试 3 次**（1s / 2s / 4s）。如仍失败：

```bash
# 1. 等几秒再试（30s 通常足够）
sleep 30 && uv run pytest tests/e2e_real -q

# 2. 加更多重试
export KIVI_LLM_MAX_RETRIES=5
...
#### 场景 D：5xx Server Error

L1/L2 已自动重试 3 次。如 3 次都失败：

- 等待几分钟后重跑（provider 端问题）
- 换 provider / 换 region
...
1. 复制 [docs/e2e-real/RESULTS_TEMPLATE.md](./docs/e2e-real/RESULTS_TEMPLATE.md)
2. 填实际数据（命令 / 路径 / 数字）
3. 重命名为 `docs/e2e-real/RESULTS.md`
4. 提交到仓库
5. （可选）把"§4 简历引用模板"直接抄到 GitHub README / 简历 / 博客

**报告模板包含**：

- 整体汇总（Provider / Model / Cases / Success Rate / Total Cost / Avg Latency）
- 5 Demo 期望输出（每段：spec / 期望 / 实际 / 评分）
- 5 Eval Case 路由决策（expected vs actual + route_match）
- 简历引用模板（直接抄）
- 复现性 checklist
- 已知问题与备注
...
- **用户文档**：[docs/e2e-real/README.md](./docs/e2e-real/README.md)（完整指南）
- **报告模板**：[docs/e2e-real/RESULTS_TEMPLATE.md](./docs/e2e-real/RESULTS_TEMPLATE.md)
- **Wave 8.2 计划**：[docs/superpowers/plans/2026-07-23-aigroup-wave8-2-real-llm-e2e.md](./docs/superpowers/plans/2026-07-23-aigroup-wave8-2-real-llm-e2e.md) §三 WT-L1 / L2 / L3
- **env var 清单**：[.env.example](./.env.example)（Wave 8.2 增 5 段）
- **测试代码**：`tests/e2e_real/test_demo{1-5}_*_e2e.py` + `test_eval_routing_e2e.py`
- **报告代码**：`tests/e2e_real/report.py`（`E2EReport` 类，JSON + Markdown）
...
### 7.2 跨大版本升级（Breaking Changes）

不适用——kivi-agent 仍在 0.0.x，v1.0.0 之前无 breaking change 承诺。

### 7.3 从 aigroup / mewcode 迁移（背景）

kivi-agent = KamaClaude 底座 + aigroup 业务能力迁移 + mewcode 44 项能力 + Wave 6.1 Vector Memory + Wave 7 演示收口 + **Wave 8.2 真实 LLM 端到端**。
...
- Wave 7 演示收口（Docker Compose + 5 演示 + RUNBOOK + README + 14 markdown）
- **Wave 8.2 真实 LLM 端到端**（L1 AnthropicProvider 增强 / L2 OpenAICompatProvider 增强 / L3 E2E 框架 / L4 文档）
...
### 7.4 Wave 7 → Wave 8 候选

按 `docs/superpowers/plans/2026-07-23-aigroup-wave7-stage-8-closure.md` §七：

- ~~**Wave 8.2**：真实 LLM 端到端（无 ANTHROPIC_API_KEY 限制的完整 E2E）~~ → **已完成**（仅框架 + 文档，真实跑由用户 export key；详见 §4）
- **Wave 8.4**：Cross-Encoder Reranker 升级 + Redis Streams Exporter

是否进入 Wave 8.1 / 8.3 取决于"对内演示 / 对外开源 / 公司生产"的最终方向（之前问过用户，**尚未收到答复**）。
