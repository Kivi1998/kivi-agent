...
from kivi_agent.core.config import KamaConfig, get_config
from kivi_agent.core.events.bus import EventBus
from kivi_agent.core.llm.factory import build_provider
from kivi_agent.core.logging_setup import setup_logging
from kivi_agent.core.mcp.server import McpServerManager
...
from kivi_agent.core.runs import events_file, new_run_id
from kivi_agent.core.session import SessionManager, SessionStore
from kivi_agent.core.trace.record import TraceRecord
from kivi_agent.core.trace.writer import TraceWriter
from kivi_agent.core.transport.ipc_broadcaster import IpcEventBroadcaster
from kivi_agent.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    ...
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        ...
        return PongResult(
            server_version=kivi_agent.__version__,
    ...
    async def _replay_events(
        ...
        if not path.exists():
            for candidate in Path("~/.kivi/sessions").expanduser().glob(
    ...
    async def run(self) -> None:
        ...
        policy_file = Path("~/.kivi/policy.toml").expanduser()
        ...
        logger.info("kivi-core %s listening addr=%s", kivi_agent.__version__, addr)
